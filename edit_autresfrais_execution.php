	...
	if (isset($_POST['submit'])) {
        ...
        if(empty($objet) || empty($unite) || empty($quantite) || empty($prix)) 
		     ...
		     $error = "Veuillez saisir des nombres pour la quantite et le prix.";
				 ...
				 editAuFraisExecution($objet, $unite, $quantite, $prix, $idde, $idfrais, $cid);

		         $_SESSION['edited_autresfrais_execution'] = 'Autre frais modifié!';
...
?>
    ...
    <center>
	     ...
	     <?php
                    if(isset($_GET['idp']) && !empty($_GET['idp']) && is_projet_exist(htmlspecialchars($_GET['idp'])) == true  && isset($_GET['idfrais']) && !empty($_GET['idfrais']) && isset($_GET['idde']) && !empty($_GET['idde'])) {
          ...
          ?>
			  ...
			  <h4>Modification frais <span style="color: #08c;"><?php echo $frais['objet_autresfrais_execution']; ?></span></h4>
 ...
 <center>
      ...
      <div class="" style="background-color:#6DCCF4; margin-top: 20px; width: 600px; border: 2px solid black; border-radius: 4px;">
          ...
          <form class="form-horizontal" action="<?php echo $_SERVER['PHP_SELF'].'?idp='.$idp.'&idde='.$idde.'&idfrais='.$idfrais; ?>" method="post" enctype="multipart/form-data" id="" style="padding-top: 20px;">
		                                
              <div class="control-group">
			  
               <label class="control-label">Objet Autres Frais:</label>
               <div class="controls" style="margin-right: 150px;">
                   <input type="text" name="objet_frais" id="objet_frais" value="<?php if($objet) echo $objet; else echo $frais['objet_autresfrais_execution']; ?>" placeholder="Entrez l'objet">
               ...
               <div class="controls" style="margin-right: 150px;">
                   <input type="text" name="unite_frais" id="unite_frais" value="<?php if($unite) echo $unite; else echo $frais['unite_autresfrais_execution']; ?>" placeholder="Entrez l'unité">
		 ...
		 <div class="control-group">
               ...
               <div class="controls" style="margin-right: 150px;">
                   <input type="text" name="quantite_frais" id="quantite_frais" value="<?php if($quantite) echo $quantite; else echo $frais['quantite_autresfrais_execution']; ?>" placeholder="Entrez la puissance du materiel roulant">
			 ...
			 <div class="control-group">
               ...
               <div class="controls" style="margin-right: 150px;">
                   <input type="text" name="prix_frais" id="prix_frais" value="<?php if($prix) echo $prix; else echo $frais['prix_autresfrais_execution']; ?>" placeholder="Entrez le prix unitaire Autres Frais">
			 ...
			 <input type="hidden" value="<?php echo $idfrais; ?>" name="idfrais">
			  ...
			  <span id="champ_cacheee">
			      ...
			      <a href="liste_autresfrais_execution.php?idp=<?php echo $idp.'&idde='.$idde; ?>">
	  ...
	  <?php 
		...
		<p>
			 ...
			 <?php
                    if(isset($_GET['idp']) && !empty($_GET['idp']) && is_projet_exist(htmlspecialchars($_GET['idp'])) == true  && isset($_GET['idde']) && !empty($_GET['idde']) && is_execution_has_donnee_autresfrais(htmlspecialchars($_GET['iddp'])) == true) {
					    				
						
          ?>
		    
		  &nbsp;<a href="liste_autresfrais_execution.php?idp=<?php echo $projet['id_projet'].'&iddp='.$idde; ?>"><button style="margin-top: 20px;" class="btn btn-primary btn-mini" type="button"> <i class="icon icon-arrow-left icon-white"></i> Retour</button></a>
