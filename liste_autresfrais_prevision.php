<?php
	...
	expired();
                    ...
                    if(isset($_GET['idp']) && !empty($_GET['idp']) && is_projet_exist(htmlspecialchars($_GET['idp'])) == true  &&  is_projet_has_donnee_prevision(htmlspecialchars($_GET['idp'])) == true && isset($_GET['iddp']) && !empty($_GET['iddp'])) {
					    ...
					    $donnees_frais = liste_donnees_autresfrais_prevision($iddp); 
                        $i = 0;
...
?>
        ...
        <?php
		    ...
		    if(isset($_SESSION['saved_autresfrais_prevision'])) {
			    echo '<em><h3 style="color: #8AC007;">'.$_SESSION['saved_autresfrais_prevision'].'</h3></em>';
				$style = 'background-color: #8AC007;';
				unset($_SESSION['saved_autresfrais_prevision']);
		    }
			
			
			
			if(isset($_SESSION['removed_autresfrais_prevision'])) {
			    echo '<em><h3 style="color: #8AC007;">'.$_SESSION['removed_autresfrais_prevision'].'</h3></em>';
				//$style = 'background-color: #8AC007;';
				unset($_SESSION['removed_autresfrais_prevision']);
		    }
			
			
			if(isset($_SESSION['edited_autresfrais_prevision'])) {
			    echo '<em><h3 style="color: #8AC007;">'.$_SESSION['edited_autresfrais_prevision'].'</h3></em>';
				//$style = 'background-color: #8AC007;';
				unset($_SESSION['edited_autresfrais_prevision']);
		    }
		...
		<h4>Autres frais - Liste des données de prevision de: <span style="color: blue;"><?php echo $prevision['nature_travaux_previsionnelle']; ?></span>
    ...
    <table border="2 solid #ccc" class="tbliste table table-striped">
        ...
        <tr style="<?php if($style) echo $style; ?>">
	        <td><?php echo $donnees_frais[$i]['id_autresfrais_previsionnel']; ?></td>
			
            <td><?php echo$donnees_frais[$i]['objet_autresfrais_previsionnel']; ?></td>
	        <td><?php echo $donnees_frais[$i]['unite_autresfrais_previsionnel']; ?></td>
            <td><?php echo $donnees_frais[$i]['quantite_autresfrais_previsionnel']; ?></td>			
	        <td><?php echo $donnees_frais[$i]['prix_autresfrais_previsionnel']; ?></td>
            <td><?php echo $donnees_frais[$i]['id_previsionnelle']; ?></td>			
	        <td>
			            <?php 
			                 $date = date_create($donnees_frais[$i]['date_saisie_autresfrais_previsionnel']);
			...
			<td>
					...
					<div class="btn-group">
                                        <a class="btn btn-info" href="#"><i class="icon- icon-white"></i> Autres frais prevision</a>
                                          
										
                                        <a class="btn dropdown-toggle" data-toggle="dropdown" href="#"><span class="caret"></span></a>
                                        <ul class="dropdown-menu">
                                        <li><a href="edit_autresfrais.php?iddp=<?php echo $iddp.'&idp='.$idp.'&idfrais='.$donnees_frais[$i]['id_autresfrais_previsionnel']; ?>"><i class="icon-pencil"></i> Modifier</a></li>
                                        <?php if(IsSuperUser($_SESSION['login'])) { ?>
                                        <li><a href="remove_autresfrais.php?iddp=<?php echo $iddp.'&idp='.$idp.'&idfrais='.$donnees_frais[$i]['id_autresfrais_previsionnel']; ?>"><i class="icon-trash"></i> Supprimer</a></li>
                                        <?php } ?>                 

										<li class="divider"></li>
                                        <li><a href="logautresfraisprevision.php?oid=<?php echo $donnees_frais[$i]['id_autresfrais_previsionnel'].'&in='.$donnees_frais[$i]['objet_autresfrais_previsionnel']; ?>"><i class=" icon-th"></i>  Historique</a></li>
