<?php
    ...
    if(isset($_GET['oid']) && !empty($_GET['oid']) && isset($_GET['in']) && !empty($_GET['in'])) { 
        ...
        $history = getLogMatroulantExecution($oid);
    	...
    	<table class="table table-bordered table-hover table-striped">
                ...
                <tbody class="tbody">
					...
					?>
                      ...
                      <td><div class="text-center">
                                              <?php 
                                                  $date = date_create($history[$i]['log_matroulant_execution_time']);
                                                 echo date_format($date, 'j F Y H:i:s'); 
                                              ?>
                     </div></td>
                            <td><div class="text-center"><?php 
                                                               
                                                               $clogin = getUserById($history[$i]['log_matroulant_execution_current_user_id']);    
                                                               echo $clogin['login']; 
                                                         ?>
                            </div></td>
                            <td><div class="text-center"><?php echo $history[$i]['log_matroulant_execution_msg']; ?></div></td>
			 ...
			 <?php
                  ...
                  echo '<h4>Accun changement effectué sur <span style="color: #08c;">'.$info.'</span></h3>'; 
						   ...
						   echo "<span style='color: red;'>Error: Aucun materiel roulant selectionné</span>";
              }						  
			 ?>
